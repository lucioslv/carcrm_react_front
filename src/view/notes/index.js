...
export default function Notes(props) {
    ...
    React.useEffect(() => {
        ...
        return () => document.getElementById('scroll') ? document.getElementById('scroll').removeEventListener('scroll', _handleScroll) : ''
                            ...
                            <div className={(state.isEdited === item.id || state.isDeleted === item.id) ? 'bg-selected' : ''}>
                                <div className="card-body d-flex align-items-center">
                                    <div className="d-none d-md-block">
                                        <Avatar className="bg-primary me-4">{item.user.name.slice(0,1)}</Avatar>
                                    </div>

                                    <div>
                                        <div className="alert alert-secondary me-4 mb-1">{item.content}</div>
                                    ...
                                    <div className="ml-auto">
                                        {(state.isDeleted === item.id) ?
                                            <CircularProgress className="me-2" color="secondary" /> :
                                            <>
